@extends('layouts.app')
@section('title', 'Admin | Alumni Pelatihan')
@section('content')
    ...
    <div class="flex flex-wrap md:flex-nowrap justify-between items-center pt-3 pb-2 mb-3 border-b border-gray-200">
        <h6 class="text-xl font-semibold text-gray-800">Daftar Alumni Pelatihan</h6>
    </div>
    ...
    <div class="card">
        <div class="controls">
            ...
            <button class="button button-success" onclick="exportToExcel()">
            ...
            <button class="button button-success"
                    data-toggle="modal" data-target="#modalTambahAlumni">
                Tambah Alumni
            </button>
        </div>
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 hidden z-50" id="modalTambahPeserta" tabindex="-1"
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-screen overflow-y-auto mb-4">
                <form action="" method="POST">
                    @csrf
                    <input type="hidden" id="id_reguler" name="id_reguler" value="">
                    <div class="bg-white rounded-lg">
                        <div class="flex justify-between items-center p-6 border-b border-gray-200">
                            <h5 class="text-xl font-semibold text-gray-800" id="modalTambahPesertaLabel">Tambah Peserta
                                Baru
                            </h5>
                        ...
                        <div class="p-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <!-- Nama Organisasi -->
                                <div class="mb-3">
                                    <label for="nama_organisasi" class="block text-sm font-medium text-gray-700 mb-2">Lembaga</label>
                                ...
                                <div class="col-span-1 md:col-span-2 mb-3">
                                    <label for="harapan_pelatihan"
                                           class="block text-sm font-medium text-gray-700 mb-2">Harapan
                                        Pelatihan</label>
    ...
    <script>
        $(document).ready(function () {
            ...
            function handleSearch() {
                ...
                if (searchTerm === '') {
                    ...
                    filteredData = allData.filter(item =>
                        ...
                        (item.lembaga ?? '').toLowerCase().includes(searchTerm) ||
            ...
            <tr class="hover:bg-gray-50 transition-colors duration-200">
                ...
                <td class="px-4 py-2 text-sm text-gray-900 editable" title="${item.email}">${item.email}</td>
                <td class="px-4 py-2 text-sm text-gray-900 editable" title="${item.no_hp}">${item.no_hp}</td>
                <td class="px-4 py-2 text-sm text-gray-900 editable" title="${item.lembaga}">${item.lembaga}</td>
                <td class="px-4 py-2 text-sm text-gray-900 editable" title="${item.jabatan}">${item.jabatan}</td>
                <td class="px-4 py-2 text-sm text-gray-900 editable" title="${item.provinsi}">${item.provinsi}</td>
                <td class="px-4 py-2 text-sm text-gray-900 editable" title="${item.kabupaten}">${item.kabupaten}</td>
                <td class="px-4 py-2 text-sm text-gray-900 editable" title="${item.aksi}><td class="px-6 py-4 whitespace-nowrap text-center">
        ...
        `).join('');
            ...
            window.exportToExcel = function () {
                try {
                    ...
                    XLSX.utils.book_append_sheet(wb, ws, 'Alumni Pelatihan');

                    const now = new Date();
                    const timestamp = now.toISOString().slice(0, 10).replace(/-/g, '');
                    const filename = `data_alumni_pelatihan_${timestamp}.xlsx`;
            ...
            // Initialize Select2 (check if available)
            if ($.fn.select2) {
            ...
            $(document).on('change', '.select-negara', function () {
                ...
                $('#id_provinsi_' + index).empty().append('<option value="">Pilih Provinsi</option>').trigger('change');
                $('#id_kabupaten_' + index).empty().append('<option value="">Pilih Kota</option>').trigger('change');
                ...
                if (negaraId) {
                    $.ajax({
                        ...
                        error: function (xhr, status, error) {
                            ...
                            showNotification('error', 'Gagal memuat data provinsi');
                        }
                    });
                }
            });
            ...
            $(document).on('change', '.select-provinsi', function () {
                ...
                if (provinsiId) {
                    $.ajax({
                        ...
                        error: function (xhr, status, error) {
                            console.error('Error:', xhr.responseText);
                            showNotification('error', 'Gagal memuat data kabupaten');
                        }
                    });
                }
            });
