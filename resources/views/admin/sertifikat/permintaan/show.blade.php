...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-lg text-green-600 font-semibold">{{ $permintaan->nama_pelatihan }}</p>
            </div>
            <div class="mt-4 sm:mt-0">
                <div class="flex items-center space-x-2 text-sm text-gray-500">
                    ...
                    <span>{{ count($permintaan->peserta_permintaan) }} Peserta</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
            ...
            <div class="flex items-center space-x-4">
                <div class="flex items-center space-x-2">
                    @php
                        $totalPeserta = count($permintaan->peserta_permintaan);
                        $sudahUpload = 0;
                        foreach($permintaan->peserta_permintaan as $p) {
                            $sertifikat = DB::table('permintaan_sertifikat')
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="overflow-x-auto">
            <table id="pesertaTable" class="w-full table-auto">
                ...
                @foreach ($permintaan->peserta_permintaan as $index => $p)
                    <tr class="hover:bg-gray-50 transition-colors table-row"
                        data-name="{{ strtolower($p->nama_peserta) }}"
                        data-peserta-id="{{ $p->id_peserta }}">
                            ...
                            <div class="flex items-center space-x-3">
                                <div class="relative flex-1">
                                    <input type="file"
                                           ...
                                           id="file_{{ $p->id_peserta }}"
                                           data-peserta-id="{{ $p->id_peserta }}"
                                           data-action="{{ route('sertiPermintaanUploadAdmin', $p->id_peserta) }}"
                                           accept=".pdf,.jpg,.jpeg,.png"
                                           class="hidden file-input">
                                    <label for="file_{{ $p->id_peserta }}"
                                    ...
                                    <div class="mt-2">
                                        <span id="filename_{{ $p->id_peserta }}"
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @if ($sertifikat)
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 status-badge"
                            ...
                            @else
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 status-badge"
                                    data-status="pending">
                                        <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div class="text-sm text-gray-700 mb-4 sm:mb-0">
                    Menampilkan <span id="showingStart">1</span> sampai <span id="showingEnd">10</span> dari <span
                        id="totalEntries">{{ count($permintaan->peserta_permintaan) }}</span> data
                </div>
                <div class="flex items-center space-x-2">
                    <button id="prevPage"
                            class="px-3 py-2 text-sm text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                        Sebelumnya
                    </button>
    ...
    <script>
        ...
        function updatePaginationButtons(totalPages) {
            ...
            if (pageNumbers) {
                ...
                for (let i = startPage; i <= endPage; i++) {
                    ...
                    pageBtn.className = `px-3 py-2 text-sm rounded-md transition-colors ${
                        i === currentPage
        ...
        function sortTable(columnIndex) {
            ...
            filteredRows.sort((a, b) => {
                ...
                if (columnIndex === 1) { // Name column
                    ...
                    const aStatus = a.querySelector('.status-badge').dataset.status;
                    const bStatus = b.querySelector('.status-badge').dataset.status;
                    aValue = aStatus === 'uploaded' ? 1 : 0;
                    bValue = bStatus === 'uploaded' ? 1 : 0;
                }
        ...
        // Make removeSelection available globally for onclick handlers
