...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Buat Pelatihan Permintaan</li>
        </ol>
    </nav>
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 ">
        <h6 class="h4">Form Tambah Pelatihan Permintaan</h6>
    </div>
    <form method="post" action="{{ route('permintaanStoreAdmin') }}" enctype="multipart/form-data">
        @csrf
        @foreach ($permintaan as $item)
            <input type="hidden" id="id_permintaan" name="id_permintaan" value="{{ $item->id_permintaan }}">
        @endforeach
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nama_pelatihan') is-invalid @enderror"
                                ...
                                value="{{ $permintaan[0]['judul_pelatihan'] ?? '' }}" autofocus>
                            ...
                            <select class="form-control" name="id_tema">
                                ...
                                @foreach ($tema as $item)
                                    <option value="{{ $item->id }}"
                                        {{ isset($permintaan[0]['id_tema']) && $permintaan[0]['id_tema'] == $item->id ? 'selected' : '' }}>
                                        {{ $item->judul_tema }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" id="tanggal_mulai" class="form-control" name="tanggal_mulai"
                                value="{{ old('tanggal_mulai', isset($permintaan[0]['tanggal_mulai']) ? \Carbon\Carbon::parse($permintaan[0]['tanggal_mulai'])->format('Y-m-d') : '') }}">
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" id="tanggal_selesai" class="form-control" name="tanggal_selesai"
                                value="{{ old('tanggal_selesai', isset($permintaan[0]['tanggal_selesai']) ? \Carbon\Carbon::parse($permintaan[0]['tanggal_selesai'])->format('Y-m-d') : '') }}">
                        </div>
