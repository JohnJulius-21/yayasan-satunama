...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item" style="color: rgb(2, 160, 2) !important;">
                <a href="{{ route('surveyKonsultasiAdmin') }}" style="color: green !important;">Survey Kepuasan Konsultasi</a>
            </li>
            <li class="breadcrumb-item active" aria-current="page">Detail Survey Kepuasan {{ $konsultasi->nama_pelatihan }}</li>
        </ol>
    </nav>
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 ">
        <h6 class="h4">Detail Hasil Survey Kepuasan Konsultasi</h6>
        <div class="d-flex justify-content-end">
            @if ($showButtons)
                <a href="{{ route('surveyCreateKonsultasiAdmin', $konsultasi->id_pelatihan_konsultasi) }}"
                    class="btn btn-success "><i style="width:17px" data-feather="plus"></i>
                    Buat Form Survey Kepuasan</a>
            @else
                <a href="{{ url('/admin/survey/edit-form-survey-konsultasi/' . $konsultasi->id_pelatihan_konsultasi) }}"
                    ...
                    <tbody>
                        @foreach ($pesertaStatus as $peserta)
                            <tr>
                                ...
                                <td>{{ $peserta->hasilSurveyKonsultasi ? 'Sudah Mengisi' : 'Belum Mengisi' }}</td>
                            </tr>
                        @endforeach
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-success">Tabel Survey Pelatihan Peserta</h6>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                @if (empty($labels))
                    <p class="text-center">Belum ada form survey yang dibuat.</p>
                @else
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <form action="{{ route('surveyDeleteKonsultasiAdmin', $konsultasi->id_pelatihan_konsultasi) }}" method="POST">
                        @csrf
                        @method('DELETE')
