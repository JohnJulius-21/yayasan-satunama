...
@section('content')
    ...
    <a href="{{ route('evaluasiPermintaanAdmin', $permintaan->id_pelatihan_permintaan)}}" class="back-icon mb-3"
       title="Kembali">
        ←
    </a>
    <!-- Breadcrumb -->
...
{{--            <li class="text-gray-700" aria-current="page">Detail Evaluasi {{ $permintaan->nama_pelatihan }}</li>--}}
{{--        </ol>--}}
{{--    </nav>--}}
    ...
    <div class="flex flex-wrap items-center justify-between pt-3 pb-2 mb-3 border-b border-gray-200">
        <h6 class="text-2xl font-medium text-gray-900">Detail Evaluasi Pelatihan Permintaan</h6>
        <div class="flex justify-end">
            @if ($showButtons)
                <a href="{{ route('evaluasiCreatePermintaanAdmin', $permintaan->id_pelatihan_permintaan) }}"
            ...
            @else
                ...
                <a href="{{ url('/admin/evaluasi/edit-form-evaluasi-permintaan/' . $permintaan->id_pelatihan_permintaan) }}"
                        ...
                        @foreach ($pesertaStatus as $peserta)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $peserta->hasilEvaluasiPermintaan ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                            {{ $peserta->hasilEvaluasiPermintaan ? 'Sudah Mengisi' : 'Belum Mengisi' }}
                                        </span>
                                </td>
                            </tr>
                        @endforeach
                ...
                @else
                    <div class="max-w-full mx-auto">
                        <table id="evaluasi" class="min-w-full divide-y divide-gray-200 border border-gray-300">
                            <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-300">Nama Peserta</th>
    ...
    <div id="deleteModal" class="fixed inset-0 flex items-center justify-center z-50 hidden">
        ...
        <div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">
            <div class="modal-content py-4 text-left px-6">
                ...
                <!-- Body -->
                <div class="mb-4">
                    <p class="text-gray-700">Apakah Anda yakin ingin menghapus form evaluasi ini? Data Evaluasi ini tidak dapat dikembalikan.</p>
                </div>

                <!-- Footer -->
                <div class="flex justify-end pt-2 space-x-3">
                    <button id="cancelDelete" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2">
                        Batal
                    </button>
                    ...
                    <form id="deleteForm" action="{{ route('evaluasiDeletePermintaanAdmin', $permintaan->id_pelatihan_permintaan) }}" method="POST" style="display: none;">
                        @csrf
                        @method('DELETE')
                    </form>
                </div>
                        ...
                        layout: {
                            topStart: {
                                buttons: [{
                                    ...
                                    text: '<svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>Export Excel'
                                }]
                            }
                        },
                        lengthChange: false,
