                ...
                <div class="col-lg-9">
                    <div class="php-email-form">
                        ...
                        {{-- @php
                            ...
                            // pastikan variabel $files adalah Collection, tidak null
                            foreach ($files as $f) {
                                // Jika elemen bukan object tapi array, gunakan $f['file_path']
                                // Ganti '/' -> '.' agar Arr::set bisa membuat nested array
                                $dotPath = str_replace('/', '.', $f->file_path);
                                Arr::set($tree, $dotPath, $f->file_url);
                            }

                            function renderTree($branch, $prefix = '')
                            {
                                foreach ($branch as $name => $node) {
                                    if (is_array($node)) {
                                        echo "<strong>{$prefix}{$name}/</strong><br>";
                                        renderTree($node, $prefix . '&nbsp;&nbsp;&nbsp;');
                                    } else {
                                        echo "{$prefix}<a href=\"{$node}\" target=\"_blank\">{$name}</a><br>";
                                    }
                                }
                            }
                        @endphp
