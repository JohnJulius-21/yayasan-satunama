<div class="mb-3 p-3 border rounded">
    ...
    @auth
        ...
        <form id="reply-form-{{ $comment->id_komen }}" action="{{ route('adminKomenStore', $comment->id_diskusi) }}" method="POST" class="d-none mt-2">
            @csrf
    ...
    @if ($comment->replies->count())
        <div class="mt-3 ms-4">
            @foreach ($comment->replies as $reply)
                @include('partials.comment-admin', ['comment' => $reply])
            @endforeach
        </div>
    @endif
</div>
